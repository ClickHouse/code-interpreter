# AppArmor Security Hardening for CodeAPI Sandbox
...
The `docker-compose.apparmor.yml` overlay tests capability-restricted mode:
...
docker compose -f docker-compose.local-dev.yml -f docker-compose.apparmor.yml up -d
./test-sandbox.sh
```
